...
外部晶振起振并稳定后，复位方可释放。复位保持为低期间，片内配置寄存器在每个 `SRC_CLK` 上升沿对 STRAP PIN 的引脚电平反复采样；复位释放时，最后一次采样的结果被冻结为本次上电的配置值，此后不可更改。

需要说明的是，芯片内部并无"等待晶振稳定"的检测电路——复位期间只要有时钟沿即会采样。保证复位释放时刻晶振已经稳定，是外部复位电路的职责，对应的时间要求见表 2-3 的 `T_rstlow`。
...
**STRAP PIN（配置引脚，又称 Bootstrap Pin / Configuration Pin）** 是指芯片在复位期间对其电平进行采样、并在复位释放时刻将采样结果锁存至内部配置寄存器的一组引脚。芯片依据锁存值确定上电后的启动行为与工作模式。

STRAP PIN 具有以下典型特征：

1. **复用性**：STRAP 功能是引脚的"第二功能"。采样完成后，该引脚立即释放并切换回其正常功能（如 UART、I²C、GPIO 等），不会因作为 STRAP 使用而损失 I/O 资源。
2. **采样与冻结**：复位保持为低期间持续采样，复位释放瞬间冻结最后一次采样值。此后引脚电平的变化不再影响已锁存的配置。
...
2. **建立/保持时间与复位时长**：STRAP PIN 电平须在复位释放沿之前稳定建立，并在复位释放沿之后继续保持。同时，复位须一直保持到外部晶振起振并稳定之后才可释放——芯片不会自行等待晶振稳定。具体要求见表 2-3。
...
1. **写序约束**：必须先写 `*_REG` 数据寄存器，再写 `TRIG` 触发位。`*_REG` 的复位值为全 0，与引脚缺省电平并不一致，因此若跳过第 1 步直接写触发位，硬件会把 0 当作有效配置加载——例如 `FUNC_MODE` 会由引脚缺省的 1 变为 0。
2. **触发位轮询约束**：向 `TRIG` 的某一位再次写 1 之前，必须先确认该位已读回 0。在触发位尚未被硬件清除时重复写入，本次请求不会产生新的上升沿，配置不会更新，且触发位可能保持为 1 不再自清。
3. **时钟频率约束**：`PCLK` 频率须**高于** `STRAP_CLK` 频率，建议留 1.5 倍以上裕量。清除脉冲宽度仅为一个 `STRAP_CLK` 周期，若 `PCLK` 过慢将无法捕获该脉冲，触发位会滞留为 1 且不再自清。
4. **采样时钟常开约束**：`STRAP_CLK` 在芯片运行期间须保持常开。若被时钟门控关断，软件重配置将静默失败——触发位置位后永不自清，配置也不会更新。
5. **回报寄存器读取时机**：`*_RPT` 为 `STRAP_CLK` 域信号，未经同步进入 `PCLK` 读通路。须在 `TRIG` 对应位读回 0 之后再读 `*_RPT`，否则可能读到配置切换过程中的中间值（`WORK_MODE` 为三位同时更新，尤需注意）。
6. **复位释放顺序约束**：`PRESETN` 的释放不应早于 `STRAP_RSTJ`。若 `PRESETN` 先释放，软件在 `STRAP_RSTJ` 仍为低时写入的触发位会在 `STRAP_RSTJ` 释放瞬间产生上升沿并立即加载，覆盖刚刚采样到的引脚值。
